...
Функционал: Проверка произвольных запросов расчета показателей
...
Сценарий: 04.02 Проверка поведения формы настройки показателей
	...
	* Проверяем поведение формы при классическом движке расчета показателей
		...
		И элемент формы с именем "ОткрытьСправкуПоЗапросам" отсутствует на форме			
	...
	* Проверяем поведение формы при расширенном движке расчета показателей
		...
		И элемент формы с именем "ОткрытьСправкуПоЗапросам" присутствует на форме			
		И я нажимаю на кнопку с именем 'ОткрытьСправкуПоЗапросам'
		Тогда открылось окно 'Использование запросов в показателях'
		И Я закрываю окно 'Использование запросов в показателях'
