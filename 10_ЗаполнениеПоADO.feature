...
Сценарий: 10.01 Загрузка методической модели
	...
	* Загрузка модели
		...
		Если '$$ЭтоУХ$$' Тогда
			И я буду выбирать внешний файл с индексом фильтра 1001 "$КаталогПроекта$\Макеты\ADO\Model_ADO.xml"
		Если '$$ЭтоЕРПУХ$$' Тогда
			И я буду выбирать внешний файл с индексом фильтра 1001 "$КаталогПроекта$\Макеты\ADO\Model_ADO_ERP.xml"
		И я нажимаю кнопку выбора у поля с именем "ИмяФайлаЗагрузки"
		Тогда открылось окно 'Обмен методическими моделями'
...
Сценарий: 10.02 Создаем экземпляр отчета
	...
	* Сверяем результат
		Тогда табличный документ "ПолеТабличногоДокументаМакет" равен:
			...
			| 'Внедрение программных продуктов '                                | 'КПГУПроф6'      | '20'         | '62 012'    | '0'          | 'КПГУПроф6'       | '30'         | '62 012'    | '0'          | 'КПГУПроф6'     | '40'         | '62 012'    | '0'          | ''        | '90'         | '0'    | '0'           |
			...
			| 'Реализация программных продуктов '                               | '4601546106674'  | '15'         | '6 030 000' | '15 340 000' | '4601546106674'   | '30'         | '6 633 000' | '33 748 000' | '4601546106674' | '45'         | '7 495 400' | '57 203 400' | ''        | '90'         | '0'    | '106 291 400' |
