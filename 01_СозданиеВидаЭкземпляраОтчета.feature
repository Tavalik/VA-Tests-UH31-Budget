...
Сценарий: 01.08 Создание экземпляра отчета
	...
	* Добавляем комментарии ячеек
		...
		И я выбираю пункт контекстного меню с именем 'ПолеТабличногоДокументаМакетКонтекстноеМенюКомментироватьПоказатель' на элементе формы с именем "ПолеТабличногоДокументаМакет"
...
Сценарий: 01.11 Отображение в дополнительной валюте
	...
	* Добавляем комментарий в отчет с дополнительной валютой
		...
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Когда открылось окно 'Экземпляр отчета ВА - ВидОтчетаСАналитиками:  Валюта отображения:  USD <Венера ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке "R47C2"
		И я выбираю пункт контекстного меню с именем 'ПолеТабличногоДокументаМакетКонтекстноеМенюКомментироватьПоказатель' на элементе формы с именем "ПолеТабличногоДокументаМакет"
		Тогда открылось окно 'Комментарии '
		И в поле с именем 'НовыйКомментарий' я ввожу текст 'Комментарий Аналитика6Валюта'
